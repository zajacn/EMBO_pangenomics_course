...
mkdir vg 
cd vg
singularity exec ../pggb.sif vg construct -r ../data/vg/tiny.fasta -v ../data/vg/tiny.vcf.gz > tiny.vg
```

Lets visualise this graph. Run both of these commands below. The difference between them is only -S.

``` bash
singularity exec ../pggb.sif vg view -dp tiny.vg | dot -T jpeg -o tiny.sequences.jpeg
singularity exec ../pggb.sif vg view -dpS tiny.vg | dot -T jpeg -o tiny.sequence_lengths.jpeg
```
...
``` bash
singularity exec ../pggb.sif vg construct -h
```
...
``` bash
singularity exec ../pggb.sif vg view tiny.vg > tiny.gfa
cat tiny.gfa
```
...
``` bash
singularity exec ../pggb.sif pggb -h
```
...
mkdir pggb
cd pggb
singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta -n 4 -t 16 -o Adh_pggb_default -S
```
...
| Run | Segment Length | Pairwise identity | Minimum match length | POA Length target | Length of the graph | Number of nodes | Number of edges |
|---------|---------|---------|---------|---------|---------|---------|---------|
| 1 | 5000 | 90 | 23 | 700,1100 | 107393 | 3214 | 4316 |
...
Lets change the pairwise identity to 60%. Check the graph statistics and then compare with the MinHash Distance (approximate divergence) [here](pairwise_dist.txt).

``` bash
singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta --map-pct-id 60 -n 4 -t 16 -o Adh_pggb_p60 -S
```
...
| Run | Segment Length | Pairwise identity | Minimum match length | POA Length target | Length of the graph | Number of nodes | Number of edges |
|---------|---------|---------|---------|---------|---------|---------|---------|
| 1 | 5000 | 90 | 23 | 700,1100 | 89769 | 10922 | 14995 |
| 2 | 5000 | 60 | 23 | 700,1100 | 89819 | 10902 | 14968 |
...
``` bash
singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta --segment-length <here_put_the_length> -n 4 -t 16 -o Adh_pggb_<define_the_name> -S
```
...
| 1 | 5000 | 90 | 23 | 700,1100 | 89769 | 10922 | 14995 |
| 2 | 5000 | 60 | 23 | 700,1100 | 89819 | 10902 | 14968 |
| 3 | 100 | 90 | 23 | 700,1100 | 101211 | 8059 | 11062 |
| 4 | 500 | 90 | 23 | 700,1100 | 92104 | 10511 | 14439 |
| 5 | 10000 | 90 | 23 | 700,1100 | 89127 | 11024 | 15147 |
| 6 | 20000 | 90 | 23 | 700,1100 | 89248 | 11105 | 15257 |
...
``` bash
singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta --segment-length <here_put_the_length> --map-pct-id 60 -n 4 -t 16 -o Adh_pggb_<define_the_name> -S
```

##### Questions

Do you see any change? Which combinations of parameters do you think performed best? Why?
...
| 5 | 10000 | 90 | 23 | 700,1100 | 89127 | 11024 | 15147 |
| 6 | 20000 | 90 | 23 | 700,1100 | 89248 | 11105 | 15257 |
| 7 | 100 | 60 | 23 | 700,1100 | 94040 | 9807 | 13466 |
| 8 | 500 | 60 | 23 | 700,1100 | 88935 | 11184 | 15369 |

: PGGB runs results

The graphs decreased in size. The best graphs are still segment-length of 5 or 10kb and the divergence of 90%.

:::

#### 3Bv. Poa-length-target

Lets now modify the last parameter, lets run more rounds of normalization and increase the target sequence length for the partial order alignment. Lets do that for the graphs created with 5,000bp and 10,000bp segment length. By default poa target length is set to 700 and 1100bp. Lets increase that and lets carry out 4 rounds of normalization.

``` bash
singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta --segment-length <here_put_the_length> --map-pct-id 90 -G 1400,1600,2000,2200 -n 4 -t 16 -o Adh_pggb_<define_the_name> -S
```
...
| 7 | 100 | 60 | 23 | 700,1100 | 94040 | 9807 | 13466 |
| 8 | 500 | 60 | 23 | 700,1100 | 88935 | 11184 | 15369 |
| 9 | 5000 | 90 | 23 | 1400,1600,2000,2200 | 93070 | 10494 | 14418 |
| 10 | 10000 | 90 | 23 | 1400,1600,2000,2200 | 92754 | 10554 | 14500 |
...
Now lets have a look at all the visualizations. They are all created with odgi viz. Here I show the default parameters graph with 4 rounds of normalization.

The default associates a color with each path. This is stable across different runs of odgi viz. (viz_multiqc.png)

![](images/Adh_chr2L.fasta.3d73c94.11fba48.34b343f.smooth.final.og.viz_multiqc.png){width="676"}
...
![](images/Adh_chr2L.fasta.3d73c94.11fba48.34b343f.smooth.final.og.viz_depth_multiqc.png)

We can look at orientation of paths using two views.

One shows the "position" of each path relative to the graph. It runs light to dark from 0 to path length. (viz_pos_multiqc.png)

![](images/Adh_chr2L.fasta.3d73c94.11fba48.34b343f.smooth.final.og.viz_pos_multiqc.png)

A similar view shows inverted regions of paths relative to the graph in red, while the forward orientation in black. (viz_inv_multiqc.png)

![](images/Adh_chr2L.fasta.3d73c94.11fba48.34b343f.smooth.final.og.viz_inv_multiqc.png)
...
![](images/Adh_chr2L.fasta.3d73c94.11fba48.34b343f.smooth.final.og.viz_O_multiqc.png)
...
``` bash
singularity exec ../pggb.sif vg construct --msa MSA.fasta > graph.vg
singularity exec ../pggb.sif vg convert -f -W graph.vg > graph.gfa
singularity exec ../pggb.sif odgi unchop -i graph.gfa -o - | singularity exec ../pggb.sif odgi sort -i - -o - | singularity exec ../pggb.sif odgi view -i - -g > graph.mod.gfa
singularity exec ../pggb.sif odgi build -g graph.mod.gfa --sort -o graph.og 
singularity exec ../pggb.sif odgi viz -i graph.og -o graph.png
```
