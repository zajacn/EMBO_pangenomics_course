...
Note your results will not be the same as my results.
:::
...
``` bash
for i in 100 500 10000 20000; do singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta --segment-length $i -n 4 -t 16 -o Adh_pggb_s${i} -S; done
```
...
``` bash
for i in 100 500; do singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta --segment-length $i --map-pct-id 60 -n 4 -t 16 -o Adh_pggb_s${i}_p60 -S; done
```
...
Lets now modify the last parameter, lets run more rounds of normalization and increase the target sequence length for the partial order alignment. Lets do that for the graphs created with 500bp and 10,000bp segment length. By default poa target length is set to 700 and 1100bp. Lets increase that and lets carry out 4 rounds of normalization.

``` bash
singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta --segment-length 10000 --map-pct-id 90 -G 1400,1600,2000,2200 -n 4 -t 16 -o Adh_pggb_s10000_G4 -S
singularity exec ../pggb.sif pggb -i ../data/pggb/Adh_chr2L.fasta --segment-length 500 --map-pct-id 60 -G 1400,1600,2000,2200 -n 4 -t 16 -o Adh_pggb_s500_G4 -S
```
