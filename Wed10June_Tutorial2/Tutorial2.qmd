...
Now lets start changing the segment length, lets test a much smaller segment length of 100bp, 500bp and then lets go with one twice and four times as much as the default - 10,000bp and 20,000bp.

``` bash
singularity exec pggb_latest.sif pggb -i data/Adh_Drosophila/chr2L_Adh.fasta --segement-length <here_put_the_length> -n 4 -t 16 -o Adh_pggb_s10k -S
```
...
What is the difference between the smaller and the larger graph lengths? Why do you think that is? 
...
with -s 100, an alignment needs 90 matching bp within a 100 bp segment,
with -s 10000, it needs 9000 matching bp across the entire segment.

So smaller segment lengths increase alignment sensitivity and create a more fragmented graph.

:::

Lets try decreasing percentage identity for the graphs with smaller segment length, lets try 60% - do you see any change?
...
The graphs decreased in size. 

:::

Which segment lengths do you think performed best? Why?

Lets now modify the last parameter, lets run more rounds of normalization and increase the target sequence length for the partial order alignment. Lets do that for the graph create with 5,000bp and 10,000bp segment length.

How does that effect graph statistics? 
...
Now lets have a look at all the visualisations. They are all created with odgi viz.
