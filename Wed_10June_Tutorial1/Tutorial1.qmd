...
The paf statistics are in data/paf_stats (```ls ../data/paf_stats``` if you want to check). Lets look at the percentage of each genome that actually mapped.
...
for (i in syri_summary_files){
  ...
  df = read.delim(paste0("../data/syri/", i), sep = "\t", header = F)
